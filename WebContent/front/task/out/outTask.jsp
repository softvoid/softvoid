...
<taskInf>
	...
	<sckid_desc>${taskBean.sckid_desc }</sckid_desc>
	<sckmc_desc>${taskBean.sckmc_desc }</sckmc_desc>
	<swldwid>${taskBean.partnerBean.swldwid }</swldwid>
	<sdwmc>${taskBean.partnerBean.sdwmc }</sdwmc>
	<sbz>${taskBean.sbz }</sbz>
	<sywymc>${taskBean.sywymc }</sywymc>
	<sfhtid>${taskBean.sfhtid }</sfhtid>
	<srwid_last>${taskBean.srwid_last }</srwid_last>
	<nrwzt>${taskBean.nrwzt }</nrwzt>
    <c:forEach var="taskItem" items="${taskBean.downItemTaskList }" >
	<inf>
		...
		<shwid_desc>${taskItem.shwid_desc }</shwid_desc>
		<srwlx>${taskItem.srwlx }</srwlx>
	</inf>
    </c:forEach>
</taskInf>