...
<body>
  ...
  <h3 align="center">批号调整查询</h3>
  <form action="<c:url value='/batchServlet' />" method="get">
    <input type="hidden" name="method" value="queryBatchNum" /> 商品编码：<input type="text" name="sspbm" value="${sspbm }" /> &nbsp;&nbsp;&nbsp;&nbsp;
    商品名称：<input type="text" name="sspmc" value="${sspmc }" /> &nbsp;&nbsp;&nbsp;&nbsp; 批号：<input type="text" name="sph" value="${sph }"> <input
  ...
  <div>
    <table align="center" width="100%" cellpadding="0px" cellspacing="0px">
      ...
      <c:forEach items="${pageList.beanList }" var="phtz">
        <tr>
          <td>${phtz.line }</td>
          <td>${phtz.njsbj }</td>
          <td>${phtz.ncklx }</td>
          <td>${phtz.drugBean.sspbm }</td>
          <td>${phtz.drugBean.sspmc }</td>
          <td>${phtz.drugBean.sspgg }</td>
          <td>${phtz.drugBean.sspcd }</td>
          <td>${phtz.sph }</td>
          <td>${phtz.dscrq }</td>
          <td>${phtz.dyxqz }</td>
          <td>${phtz.n4sl }</td>
